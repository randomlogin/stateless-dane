...
async function getDnssecChain(name) {
  ...
  // let output = await exec(`delv @easyhandshake.com/dns-query -p 53 -a ${kskPath} _443._tcp.${name} TLSA +rtrace +mtrace +nosplit`);
  let output = await exec(`delv @127.0.0.1 -p 53 -a ${kskPath} _777._tcp.${name} TLSA +rtrace +mtrace +nosplit`);
  // let output = await exec(`delv @127.0.0.1 -p 53 -a ${kskPath} _443._tcp.${name} TLSA +rtrace +mtrace +nosplit`);

  if (!output.stdout.includes('; fully validated')) { return null;
  }
